    ...
    <section style={{ padding: '80px 0', overflow: 'hidden', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', animation: 'marquee 30s linear infinite', width: 'max-content' }}>
          {partners.map((p, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '0 40px', whiteSpace: 'nowrap' }}>
              <div style={{ width: '8px', height: '8px', background: 'var(--orange)', borderRadius: '50%', opacity: 0.5 }} />
              <span style={{ fontFamily: 'var(--font-syne)', fontSize: '16px', fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '-0.01em' }}>{p}</span>
            </div>
          ))}
        </div>
      </div>
